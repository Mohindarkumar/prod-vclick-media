  ...
  return (
    <div className="relative h-auto sm:h-full w-full flex items-center justify-center px-6 sm:px-10 md:px-14 lg:px-16 py-12 sm:py-14 md:py-16">
      ...
      >
        ...
        <m.div variants={item} className="relative w-20 h-20 flex items-center justify-center mb-6" aria-hidden="true">
          <m.div
        ...
        <m.span variants={item} className="eyebrow">Who We Are</m.span>
        <m.h3 variants={item} className="mt-4 text-[clamp(1.75rem,4vw,2.75rem)] font-extrabold text-paper leading-[1.1]">
          {data.title}
        </m.h3>
        <m.span variants={item} className="mt-4 block h-[3px] w-14 rounded-full bg-gold-sweep" aria-hidden="true" />
        <m.p variants={item} className="mt-6 text-[clamp(0.95rem,1.4vw,1.125rem)] text-mist leading-relaxed">
          {data.description}
        </m.p>
        {data.cta && (
